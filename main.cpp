...
void newCreateRelationForward(int newRelationSize);
void newCreateRelationBackward(int newRelationSize);
void newCreateRelationRandom(int newRelationSize);
void newIndexTests();
void intTests();
void newIntTests();
...
void test2();
void test3();
void test4(int newRelationSize);
void test5(int newRelationSize);
void test6(int newRelationSize);
void test7();
void errorTests();
void deleteRelation();
...
int main(int argc, char **argv)
{
	...
	test2();
	test3();
	int newRelationSize = 100000;
	test4(newRelationSize);
	test5(newRelationSize);
	test6(newRelationSize);
	test7();
...
void test4(int newRelationSize)
{
	...
	std::cout << "---------------------" << std::endl;
	std::cout << "createRelationForward" << std::endl;
	newCreateRelationForward(newRelationSize);
	indexTests();
	deleteRelation();
}
...
void test5(int newRelationSize)
{
	...
	std::cout << "----------------------" << std::endl;
	std::cout << "createRelationBackward" << std::endl;
	newCreateRelationBackward(newRelationSize);
	indexTests();
	deleteRelation();
}
...
void test6(int newRelationSize)
{
	...
	std::cout << "--------------------" << std::endl;
	std::cout << "createRelationRandom" << std::endl;
	newCreateRelationRandom(newRelationSize);
	indexTests();
	deleteRelation();
}
void test7()
...
void newIntTests()
{
	...
	// extra scan test
	// check whether scan stops after reach the last leafnode
	checkPassFail(intScan(&index, -1, GTE, 6000, LTE), 5000)
			// check wheter correct result for same low and high value
...
void newCreateRelationForward(int newRelationSize)
{
	...
	// Insert a bunch of tuples into the relation.
	for (int i = 0; i < newRelationSize; i++)
	{
		sprintf(record1.s, "%05d string record", i);
		record1.i = i;
		record1.d = (double)i;
...
void newCreateRelationBackward(int newRelationSize)
{
	...
	// Insert a bunch of tuples into the relation.
	for (int i = newRelationSize - 1; i >= 0; i--)
	{
		sprintf(record1.s, "%05d string record", i);
		record1.i = i;
		record1.d = i;
...
void newCreateRelationRandom(int newRelationSize)
{
	...
	std::vector<int> intvec(newRelationSize);
	for (int i = 0; i < newRelationSize; i++)
	{
		intvec[i] = i;
	}
	...
	int val;
	int i = 0;
	while (i < newRelationSize)
	{
		pos = random() % (newRelationSize - i);
		val = intvec[pos];
		sprintf(record1.s, "%05d string record", val);
		record1.i = val;
		record1.d = val;
		...
		int temp = intvec[newRelationSize - 1 - i];
		intvec[newRelationSize - 1 - i] = intvec[pos];
		intvec[pos] = temp;
		i++;
	}
